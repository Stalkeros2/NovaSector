/datum/atc_chatter/traveladvisory/squak()
	...
	switch(phase)
		if(1)
			SSatc.msg("[callname], all vessels in the Nova Sector system. Priority travel advisory follows.")
			next()
		else
...
/datum/atc_chatter/pathwarning/squak()
	...
	switch(phase)
		...
		if(2)
			...
			SSatc.msg("[confirm] [callname], adjusting course.")
			next()
		else
...
/datum/atc_chatter/civvieleaks/squak()
	...
	switch(phase)
		if(1)
			...
			switch(commleak)
				if("thatsmywife")
					SSatc.msg("-so then I says to him, |that's no [pick("space carp","space shark","vox","garbage scow","freight liner","cargo hauler","superlifter")], that's my +wife!+| And he-")
					next()
				if("missingkit")
					SSatc.msg("-did you get the kit from down on deck [rand(1,4)]? I need th-")
					next()
				if("pipeleaks")
					SSatc.msg("I swear if these pipes keep leaking I'm going to-")
					next()
				if("weirdsmell")
					SSatc.msg("-and where the hell is that smell coming fr-")
					next()
				if("weirdsmell2")
					SSatc.msg("-hat in the [pick("three","five","seven","nine")] hells did you |eat| [pick("ensign","crewman")]? This compartment reeks of-")
					next()
				if("scug")
					SSatc.msg("-and if that weird cat of yours keeps crawling into the pipes we-")
					next()
				if("teppi")
					SSatc.msg("-at are we supposed to do with this damn cow?")
...
/datum/atc_chatter/slogan/squak()
	switch(phase)
		if(1)
			SSatc.msg("The following is a sponsored message from [name].")
			next()
		else
			SSatc.msg("[slogan]")
			finish()
...
/datum/atc_chatter/misc/squak()
	...
	switch(phase)
		if(1)
			SSatc.msg("[callname], [combined_first_name] on [mission] [pick(mission_noun)] to [destname], requesting [request].")
			next()
		if(2)
			SSatc.msg("[combined_first_name], [callname], [response].")
			next()
		else
			SSatc.msg("[callname], [yes ? "thank you" : "understood"], out.")
			finish()

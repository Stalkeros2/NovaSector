...
/datum/atc_chatter/policeflee/squak()
	...
	switch(phase)
		...
		if(2)
			...
			SSatc.msg("Nova Sector Defense Control to all local assets: vector to interdict and detain [prefix], temporary callsign |[shipname]|. Control out.","Nova Sector Defense Control")
			finish()
...
/datum/atc_chatter/policeshipflee/squak()
	...
	switch(phase)
		...
		if(2)
			...
			next()
		if(3)
			SSatc.msg("Nova Sector Defense Control,  [combined_second_name]. We have a [prefix] here, please advise.","[comm_second_name]")
			next()
		else
			SSatc.msg("Defense Control copies, [combined_second_name], reinforcements are en route. Switch further communications to encrypted band [SSatc.sdfchannel].","Nova Sector Defense Control")
			finish()
...
/datum/atc_chatter/policeshipcombat/squak()
	...
	switch(phase)
		if(1)
			...
			SSatc.msg("Nova Sector Defense Control,  [combined_second_name], engaging [combined_first_name] [pick("near route","in sector")] [rand(1,100)], [battlestatus]","[comm_second_name]")
			next()
		else
			SSatc.msg("Nova Sector Defense Control copies, [combined_second_name]. Keep us updated.","Nova Sector Defense Control")
			finish()
...
/datum/atc_chatter/hostiledetected/squak()
	...
	switch(phase)
		if(1)
			SSatc.msg("This is Nova Sector Defense Control to all SDF assets. Priority update follows.","Nova Sector Defense Control")
			next()
		else
			...
			SSatc.msg("Be on the lookout for [short_first_name], last sighted [pick("near route","in sector","near sector")] [rand(1,100)]. [orders]. DefCon, out.","Nova Sector Defense Control")
			finish()

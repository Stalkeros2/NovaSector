...
/datum/atc_chatter/distress/squak()
	...
	switch(state)
		if("calm")
			switch(phase)
				if(1)
					SSatc.msg("Nova Sector Defense Control, [combined_first_name].","[comm_first_name]")
					next()
				if(2)
					SSatc.msg("[combined_first_name], Nova Sector Defense Control.","Nova Sector Defense Control")
					next()
				if(3)
					...
					next()
				if(4)
					SSatc.msg("[combined_first_name], Nova Sector Defense Control copies. Sensor data matches logged profile for [secondprefix] |[secondshipname]|. SDF units are en route to your location.","Nova Sector Defense Control")
					next()
				else
		...
		if("panic")
			switch(phase)
				if(1)
					...
					next()
				if(2)
					SSatc.msg("[combined_first_name], Nova Sector Defense Control. SDF is en route, contact on [SSatc.sdfchannel].")
					next()
				else
					SSatc.msg("[pick("Copy that","Understood")] Nova Sector Defense Control, switching now!","[comm_first_name]")
					finish()

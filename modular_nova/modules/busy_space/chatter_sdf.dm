/datum/atc_chatter/sdfpatrolupdate/squak()
	...
	switch(phase)
		if(1)
			...
			SSatc.msg("Nova Sector Defense Control,  [combined_first_name] reporting in, [statusupdate], over.","[comm_first_name]")
			next()
		else
			SSatc.msg("Nova Sector Defense Control copies, [combined_first_name]. Keep us updated, out.","Nova Sector Defense Control")
			finish()
		...
		if("commscheck")
			switch(phase)
				if(1)
					...
					next()
				if(2)
					SSatc.msg("Control reads you loud and clear [combined_first_name].","Nova Sector Defense Control")
					next()
				else

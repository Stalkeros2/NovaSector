...
/obj/item/clothing/head/helmet/cin_martyr/dropped(mob/living/carbon/human/user)
	. = ..()
	UnregisterSignal(user, list(COMSIG_ITEM_ATTACK, COMSIG_ITEM_ATTACK_ATOM, COMSIG_ITEM_HIT_REACT, COMSIG_PROJECTILE_PREHIT))
...
/obj/item/clothing/head/helmet/cin_martyr/proc/apply_damage(mob/living/carbon/human/wearer)
	...
	if(remaining_damage <= 0)
		...
		damage_timer_id = addtimer(CALLBACK(src, PROC_REF(apply_damage), wearer), 1 SECONDS, TIMER_STOPPABLE)
		if(!processing)
			processing = TRUE
			START_PROCESSING(SSobj, src)

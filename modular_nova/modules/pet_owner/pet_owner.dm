...
/datum/quirk_constant_data/pet_owner
	...
	customization_options = list(
		...
		/datum/preference/text/pet_ghost_behavior,
		/datum/preference/toggle/pet_ghost_controlable,
	)
...
/datum/quirk/item_quirk/pet_owner/add_unique(client/client_source)
	...
	var/ghost_control = client_source?.prefs.read_preference(/datum/preference/toggle/pet_ghost_controlable)
	var/ghost_behavior = client_source?.prefs.read_preference(/datum/preference/text/pet_ghost_behavior)
	...
	if (!isnull(ghost_control))
		var/mob/living/chosen_one = SSpolling.poll_ghosts_for_target(check_jobban = ROLE_SENTIENCE, role = ROLE_SENTIENCE, poll_time = 30 SECONDS, checked_target = pet, ignore_category = POLL_IGNORE_SENTIENCE_POTION, alert_pic = src, role_name_text = "[quirk_holder.real_name]'s pet")
		if(isnull(chosen_one))
			pet.balloon_alert(pet, "remained stupid!")
		else
			pet.balloon_alert(pet, "became smart!")
		if(chosen_one.key)
			pet.key = chosen_one.key
			pet.mind.enslave_mind_to_creator(quirk_holder)
			pet.mind.add_antag_datum(/datum/antagonist/sentient_creature)
			to_chat(pet, "<b>You have been chosen to play as [quirk_holder.real_name]'s pet.\n\
			This is the way they wish you to act as: [ghost_behavior]!</b>")
	give_item_to_holder(
		carrier,
		list(
...
/datum/preference/text/pet_ghost_behavior/is_accessible(datum/preferences/preferences)
	if (!..())
		return FALSE

	return "Pet Owner" in preferences.all_quirks

/datum/preference/text/pet_ghost_behavior/serialize(input)
	return htmlrendertext(input)

/datum/preference/text/pet_ghost_behavior/apply_to_human(mob/living/carbon/human/target, value)
	return
...
/datum/preference/toggle/pet_ghost_controlable
	category = PREFERENCE_CATEGORY_MANUALLY_RENDERED
	savefile_key = "pet_ghost_controlable"
	savefile_identifier = PREFERENCE_CHARACTER
	can_randomize = FALSE
	default_value = FALSE

/datum/preference/toggle/pet_ghost_controlable/is_accessible(datum/preferences/preferences)
	if (!..())
		return FALSE

	return "Pet Owner" in preferences.all_quirks

/datum/preference/toggle/pet_ghost_controlable/apply_to_human(mob/living/carbon/human/target, value)
	return
